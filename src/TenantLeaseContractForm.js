...
const TenantLeaseContractForm = ({connectedContract }) => {
    ...
    const [pdfFile, setPdfFile] = useState(null); // Lease agreement PDF file
    const [evacuationPdf, setEvacuationPdf] = useState(null); // Evacuation agreement PDF file

    const handleFileChange = (e, setFileState) => {
        const file = e.target.files[0];
        if (file && file.type === 'application/pdf') {
            setFileState(file);
            setError(''); // Clear any existing errors when a valid file is selected
        } else {
    ...
    const handleDeploy = async () => {
        ...
        if (!evacuationPdf) {
            setError('Please upload the evacuation agreement (Tahliye Taahhütnamesi).');
            return;
        }
    ...
    return (
        <div style={styles.container}>
            <h1 style={styles.header}>Lease Agreement Deployment For Tenant</h1>
            ...
            <div style={styles.formContainer}>
                {!deploymentState.isDeployed ? (
                    <>
                        ...
                        <div style={styles.inputGroup}>
                            <label style={styles.label}>Upload Evacuation Agreement (Tahliye Taahhütnamesi)
                                (PDF):</label>
                            <input
                                type="file"
                                accept="application/pdf"
                                aria-label="Evacuation Agreement Upload"
                                onChange={(e) => handleFileChange(e, setEvacuationPdf)}
                                style={styles.input}
                            />
                            {evacuationPdf && <p style={styles.fileInfo}>Selected file: {evacuationPdf.name}</p>}
                        </div>
...
export default TenantLeaseContractForm;